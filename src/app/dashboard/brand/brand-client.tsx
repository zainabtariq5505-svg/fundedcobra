...
import { Palette, Search, Filter, Settings2, MoreVertical, Plus } from "lucide-react";
...
export default function BrandClient({ data, stats }: { data: BrandAssets[]; stats: { total: number; withLogo: number; withBanner: number } }) {
  const [searchTerm, setSearchTerm] = useState("");
  const filtered = data.filter(b => b.guildId.toLowerCase().includes(searchTerm.toLowerCase()));
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">Manage server banners, logos, and dashboard theme.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Total Guilds", value: stats.total.toString(), trend: "Configured" },
          { label: "With Logo", value: stats.withLogo.toString(), trend: "Custom" },
          { label: "With Banner", value: stats.withBanner.toString(), trend: "Custom" },
        ].map((stat, i) => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead><tr className="bg-white/5 text-gray-400 text-xs uppercase tracking-wider">
              ...
              <th className="p-4 font-medium">Brand Color</th>
              <th className="p-4 font-medium">Footer Text</th>
              <th className="p-4 font-medium">Updated</th>
              <th className="p-4 font-medium"></th>
            </tr></thead>
            <tbody className="divide-y divide-white/5">
              {filtered.length === 0 ? (
                <tr><td colSpan={5} className="p-8 text-center text-gray-500">No brand assets found.</td></tr>
              ) : filtered.map((item) => (
                <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-white/[0.02] transition-colors group cursor-pointer">
                  ...
                  <td className="p-4 text-sm text-gray-300"><span className="inline-flex items-center gap-2"><span className="w-3 h-3 rounded-full" style={{ backgroundColor: item.brandColor || '#7c3aed' }}></span>{item.brandColor || "Default"}</span></td>
                  <td className="p-4 text-sm text-gray-300">{item.footerText || "—"}</td>
