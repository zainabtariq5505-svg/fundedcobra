...
import { UserCog, Search, Filter, Settings2, MoreVertical, Plus } from "lucide-react";
...
export default function AdminsClient({ 
  ...
  data: AdminUser[];
  stats: { total: number; active: number; adminRoles: number }
}) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">Manage dashboard access and staff accounts.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Total Admins", value: stats.total.toString(), trend: "Registered" },
          { label: "Active", value: stats.active.toString(), trend: "Not disabled" },
          { label: "Admin Role", value: stats.adminRoles.toString(), trend: "Super admins" },
        ].map((stat, i) => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5">
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="p-8 text-center text-gray-500">No admin users found.</td></tr>
              ) : filtered.map((item) => (
                <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-white/[0.02] transition-colors group cursor-pointer">
                  ...
                  <td className="p-4 text-sm text-gray-300 capitalize">{item.role}</td>
                  <td className="p-4 text-sm text-gray-400">{item.lastLoginAt ? new Date(item.lastLoginAt).toLocaleDateString() : "Never"}</td>
                  <td className="p-4">
                    <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium border ${item.disabled ? 'text-red-400 bg-red-400/10 border-red-400/20' : 'text-green-400 bg-green-400/10 border-green-400/20'}`}>
                      {item.disabled ? "Disabled" : "Active"}
                    </span>
                  </td>
                  <td className="p-4 text-right">
                    <button className="text-gray-500 hover:text-white transition-colors opacity-0 group-hover:opacity-100"><MoreVertical size={18} /></button>
                  </td>
                </motion.tr>
              ))}
            </tbody>
