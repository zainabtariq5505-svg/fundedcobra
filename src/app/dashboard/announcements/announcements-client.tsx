...
import { Megaphone, Search, Filter, Settings2, MoreVertical, Plus } from "lucide-react";
...
export default function AnnouncementsClient({ data, stats }: { data: Announcement[]; stats: { total: number; scheduled: number; sent: number } }) {
  const [searchTerm, setSearchTerm] = useState("");
  const filtered = data.filter(a => a.title.toLowerCase().includes(searchTerm.toLowerCase()) || a.type.toLowerCase().includes(searchTerm.toLowerCase()));
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight flex items-center gap-3">
            <Megaphone className="text-purple-500" size={32} />
            Announcements
          </h1>
          <p className="text-gray-400 mt-1">Draft and schedule server announcements.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          { label: "Scheduled", value: stats.scheduled.toString(), trend: "Pending" },
          { label: "Sent", value: stats.sent.toString(), trend: "Delivered" },
        ].map((stat, i) => (
      ...
      <div className="rounded-2xl bg-[#111116] border border-white/5 overflow-hidden">
        <div className="p-4 border-b border-white/5 flex items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" placeholder="Search by title or type..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5">
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="p-8 text-center text-gray-500">No announcements found.</td></tr>
              ) : filtered.map((item) => (
                <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-white/[0.02] transition-colors group cursor-pointer">
                  <td className="p-4 text-sm font-medium text-white max-w-xs truncate">{item.title}</td>
                  <td className="p-4 text-sm text-gray-300">{item.type}</td>
                  <td className="p-4"><span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium border ${item.status === 'sent' ? 'text-green-400 bg-green-400/10 border-green-400/20' : 'text-yellow-400 bg-yellow-400/10 border-yellow-400/20'}`}>{item.status}</span></td>
                  <td className="p-4 text-sm text-gray-400">{item.scheduledFor ? new Date(item.scheduledFor).toLocaleDateString() : "—"}</td>
                  <td className="p-4 text-sm text-gray-300">{item.authorId}</td>
