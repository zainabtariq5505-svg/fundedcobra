...
import { UserPlus, Search, Filter, Settings2, MoreVertical, Plus } from "lucide-react";
...
export default function WelcomeClient({ logs, settings, stats }: { logs: WelcomeLog[]; settings: WelcomeSettings[]; stats: { total: number; enabled: number; todayCount: number } }) {
  ...
  const filteredSettings = settings.filter(s => s.guildId.toLowerCase().includes(searchTerm.toLowerCase()));
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">Configure welcome messages and automatic roles.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Total Welcomes", value: stats.total.toString(), trend: "All time" },
          { label: "Enabled", value: stats.enabled.toString(), trend: "Guilds" },
          { label: "Today", value: stats.todayCount.toString(), trend: "New joins" },
        ].map((stat, i) => (
      ...
      <div className="flex gap-4 mb-2">
        <button onClick={() => setTab('logs')} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${tab === 'logs' ? 'bg-purple-600 text-white' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}>Logs</button>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead><tr className="bg-white/5 text-gray-400 text-xs uppercase tracking-wider">
              {tab === 'logs' ? (
                <><th className="p-4 font-medium">User ID</th><th className="p-4 font-medium">Guild ID</th><th className="p-4 font-medium">Created</th><th className="p-4 font-medium"></th></>
              ) : (
                <><th className="p-4 font-medium">Guild ID</th><th className="p-4 font-medium">Enabled</th><th className="p-4 font-medium">Channel</th><th className="p-4 font-medium">Updated</th><th className="p-4 font-medium"></th></>
              )}
            </tr></thead>
            <tbody className="divide-y divide-white/5">
              {tab === 'logs' ? (
                filteredLogs.length === 0 ? (
                  <tr><td colSpan={4} className="p-8 text-center text-gray-500">No welcome logs found.</td></tr>
                ...
                filteredSettings.length === 0 ? (
                  <tr><td colSpan={5} className="p-8 text-center text-gray-500">No welcome settings found.</td></tr>
