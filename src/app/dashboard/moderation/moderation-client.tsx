...
export default function ModerationClient({
  ...
  initialCases: ModCase[];
  stats: { activeBans: number; activeMutes: number; warningsGiven: number; autoModActions: number }
}) {
  const [activeTab, setActiveTab] = useState("all");
  const [searchQuery, setSearchQuery] = useState("");

  const filteredCases = initialCases.filter(c => {
    if (activeTab !== "all" && c.actionType.toLowerCase() !== activeTab.toLowerCase()) return false;
    if (searchQuery && !c.userId.toLowerCase().includes(searchQuery.toLowerCase()) && !c.moderatorId.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: "Active Bans", value: stats.activeBans.toString(), trend: "Live", color: "text-red-400" },
          { label: "Active Mutes", value: stats.activeMutes.toString(), trend: "Live", color: "text-orange-400" },
          { label: "Warnings Given", value: stats.warningsGiven.toString(), trend: "All time", color: "text-yellow-400" },
          { label: "AutoMod Actions", value: stats.autoModActions.toString(), trend: "Blocked", color: "text-purple-400" },
        ].map((stat, i) => (
          <motion.div
            key={i}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between p-1 bg-white/5 rounded-xl border border-white/5 w-fit">
            {['all', 'ban', 'mute', 'warn', 'kick'].map(tab => (
              <button
                key={tab}
          ...
          <div className="relative">
            ...
            <input 
              type="text" 
              placeholder="Search by User ID or Moderator ID..." 
              value={searchQuery}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/5">
                  ...
                  ) : filteredCases.map((action) => {
                    const elapsedMins = Math.floor((Date.now() - new Date(action.createdAt).getTime()) / 60000);
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-gray-800 border border-gray-700 flex items-center justify-center text-xs text-gray-400">
                              {action.userId.substring(0, 2)}
                            </div>
                            <span className="font-medium text-gray-200">{action.userId}</span>
                          </div>
                        </td>
                        <td className="p-4">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold border uppercase ${getActionColor(action.actionType)}`}>
