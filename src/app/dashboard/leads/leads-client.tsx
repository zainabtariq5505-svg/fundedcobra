...
import { Target, Search, Filter, Settings2, MoreVertical, Plus } from "lucide-react";
...
export default function LeadsClient({ data, stats }: { data: Lead[]; stats: { total: number; newLeads: number; hotLeads: number } }) {
  const [searchTerm, setSearchTerm] = useState("");
  const filtered = data.filter(l => l.userId.toLowerCase().includes(searchTerm.toLowerCase()) || l.status.toLowerCase().includes(searchTerm.toLowerCase()));
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">Manage hot leads, sales stages, and follow-ups.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Total Leads", value: stats.total.toString(), trend: "All time" },
          { label: "New", value: stats.newLeads.toString(), trend: "Fresh" },
          { label: "Hot", value: stats.hotLeads.toString(), trend: "High priority" },
        ].map((stat, i) => (
      ...
      <div className="rounded-2xl bg-[#111116] border border-white/5 overflow-hidden">
        <div className="p-4 border-b border-white/5 flex items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" placeholder="Search by user ID or status..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead><tr className="bg-white/5 text-gray-400 text-xs uppercase tracking-wider">
              ...
              <th className="p-4 font-medium">Sales Stage</th>
            ...
            <tbody className="divide-y divide-white/5">
              {filtered.length === 0 ? (
                ...
                <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-white/[0.02] transition-colors group cursor-pointer">
                  ...
                  <td className="p-4 text-sm text-gray-300">{item.score}</td>
                  <td className="p-4"><span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium border ${item.status === 'hot' ? 'text-red-400 bg-red-400/10 border-red-400/20' : item.status === 'warm' ? 'text-orange-400 bg-orange-400/10 border-orange-400/20' : 'text-blue-400 bg-blue-400/10 border-blue-400/20'}`}>{item.status}</span></td>
                  <td className="p-4 text-sm text-gray-300">{item.salesStage}</td>
                  <td className="p-4 text-sm text-gray-400">{item.sourceChannel || "—"}</td>
                  <td className="p-4 text-sm text-gray-400">{new Date(item.lastActive).toLocaleDateString()}</td>
