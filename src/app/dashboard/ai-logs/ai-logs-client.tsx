...
import { BrainCircuit, Search, Filter, Settings2, MoreVertical, Plus } from "lucide-react";
...
export default function AiLogsClient({ data, stats }: { data: QuestionLog[]; stats: { total: number; needsHuman: number; avgConfidence: number } }) {
  const [searchTerm, setSearchTerm] = useState("");
  const filtered = data.filter(l => l.question.toLowerCase().includes(searchTerm.toLowerCase()) || (l.intent && l.intent.toLowerCase().includes(searchTerm.toLowerCase())));
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">Monitor bot AI interactions and confidence scores.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Total Interactions", value: stats.total.toString(), trend: "All time" },
          { label: "Needs Human", value: stats.needsHuman.toString(), trend: "Escalated" },
          { label: "Avg Confidence", value: `${(stats.avgConfidence * 100).toFixed(1)}%`, trend: "Score" },
        ].map((stat, i) => (
      ...
      <div className="rounded-2xl bg-[#111116] border border-white/5 overflow-hidden">
        <div className="p-4 border-b border-white/5 flex items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" placeholder="Search by question or intent..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead><tr className="bg-white/5 text-gray-400 text-xs uppercase tracking-wider">
              ...
              <th className="p-4 font-medium">Needs Human</th>
              <th className="p-4 font-medium">Created</th>
              <th className="p-4 font-medium"></th>
            </tr></thead>
            <tbody className="divide-y divide-white/5">
              {filtered.length === 0 ? (
                <tr><td colSpan={7} className="p-8 text-center text-gray-500">No AI logs found.</td></tr>
              ) : filtered.map((item) => (
                <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-white/[0.02] transition-colors group cursor-pointer">
                  ...
                  <td className="p-4 text-sm text-gray-300 max-w-xs truncate">{item.question}</td>
                  <td className="p-4 text-sm text-gray-300">{item.intent || "—"}</td>
                  <td className="p-4 text-sm text-gray-300">{item.confidence ? `${(item.confidence * 100).toFixed(1)}%` : "—"}</td>
                  <td className="p-4"><span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium border ${item.needsHuman ? 'text-red-400 bg-red-400/10 border-red-400/20' : 'text-green-400 bg-green-400/10 border-green-400/20'}`}>{item.needsHuman ? "Yes" : "No"}</span></td>
