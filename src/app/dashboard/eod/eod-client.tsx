...
import { Clock, Coffee, Play, CheckCircle2, ChevronRight, Send, Search, Filter } from "lucide-react";
...
export default function EodClient({ data, stats }: { data: EmployeeEOD[]; stats: { total: number; todayCount: number; totalTickets: number; totalLeads: number } }) {
  ...
  const handleAction = (newState: WorkState) => {
    ...
    if (newState === "clocked_out") setIsSubmitModalOpen(true);
  };
  ...
  const filteredEODs = data.filter(e => e.userId.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="space-y-6 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 space-y-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white/5 border border-white/5 backdrop-blur-xl p-8 rounded-2xl relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
              ...
              <div className="grid grid-cols-2 gap-4 w-full md:w-auto">
                {workState === "not_started" && (
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleAction("working")}
                ...
                {workState === "working" && (
                  <>
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleAction("on_lunch")}
                      ...
                      <Coffee size={24} className="text-orange-400" /> <span className="text-sm">Start Lunch</span>
                    </motion.button>
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleAction("clocked_out")}
                      ...
                      <CheckCircle2 size={24} /> <span className="text-sm">Clock Out</span>
                    </motion.button>
                  </>
                )}
                {workState === "on_lunch" && (
                ...
                {workState === "clocked_out" && (
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setIsSubmitModalOpen(true)}
          ...
          <div className="bg-white/5 border border-white/5 backdrop-blur-xl p-6 rounded-2xl">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center p-3 rounded-lg bg-white/5 border border-white/5">
                ...
                <span className="font-bold text-white">{stats.totalTickets}</span>
              </div>
              <div className="flex justify-between items-center p-3 rounded-lg bg-white/5 border border-white/5">
                <span className="text-sm text-gray-400">Leads Followed</span>
                <span className="font-bold text-white">{stats.totalLeads}</span>
              </div>
              <div className="flex justify-between items-center p-3 rounded-lg bg-white/5 border border-white/5">
                <span className="text-sm text-gray-400">EODs Today</span>
                <span className="font-bold text-white">{stats.todayCount}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-2xl bg-[#111116] border border-white/5 overflow-hidden mt-6">
        <div className="p-4 border-b border-white/5 flex items-center justify-between gap-4">
          <h3 className="text-lg font-medium text-white">Recent EOD Reports</h3>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5">
              {filteredEODs.length === 0 ? (
                <tr><td colSpan={6} className="p-8 text-center text-gray-500">No EOD reports found.</td></tr>
                ...
                <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-white/[0.02] transition-colors group cursor-pointer">
                  ...
                  <td className="p-4 text-sm text-gray-300">{item.ticketsHandled}</td>
                  <td className="p-4 text-sm text-gray-300">{item.leadsFollowedUp}</td>
                  <td className="p-4 text-sm text-gray-400">{new Date(item.submittedAt).toLocaleString()}</td>
                  <td className="p-4 text-right"><button className="text-gray-500 hover:text-white transition-colors opacity-0 group-hover:opacity-100"><ChevronRight size={18} /></button></td>
                </motion.tr>
              ))}
            </tbody>
          </table>
      ...
      <AnimatePresence>
        {isSubmitModalOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setIsSubmitModalOpen(false)} />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
              ...
              <form onSubmit={handleSubmitEOD} className="p-6 space-y-5 max-h-[70vh] overflow-y-auto custom-scrollbar">
                <div>
                  ...
                  <textarea required value={eodForm.workDone} onChange={(e) => setEodForm({...eodForm, workDone: e.target.value})}
                    className="w-full bg-black/50 border border-white/10 rounded-xl p-3 text-white focus:ring-2 focus:ring-purple-500 outline-none h-24" placeholder="Brief summary of your daily tasks..." />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1">Tickets Handled</label>
                    <input type="number" value={eodForm.ticketsHandled} onChange={(e) => setEodForm({...eodForm, ticketsHandled: parseInt(e.target.value)})}
                  ...
                  <div>
                    ...
                    <input type="number" value={eodForm.leadsFollowedUp} onChange={(e) => setEodForm({...eodForm, leadsFollowedUp: parseInt(e.target.value)})}
                      className="w-full bg-black/50 border border-white/10 rounded-xl p-3 text-white focus:ring-2 focus:ring-purple-500 outline-none" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Blockers / Issues</label>
                  <textarea value={eodForm.blockers} onChange={(e) => setEodForm({...eodForm, blockers: e.target.value})}
                    className="w-full bg-black/50 border border-white/10 rounded-xl p-3 text-white focus:ring-2 focus:ring-purple-500 outline-none h-20" placeholder="Anything slowing you down?" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Plan for Tomorrow</label>
                  <textarea required value={eodForm.tomorrowPlan} onChange={(e) => setEodForm({...eodForm, tomorrowPlan: e.target.value})}
                    className="w-full bg-black/50 border border-white/10 rounded-xl p-3 text-white focus:ring-2 focus:ring-purple-500 outline-none h-20" placeholder="What will you focus on tomorrow?" />
                </div>
                <div className="pt-4 border-t border-white/10 flex justify-end gap-3">
                  <button type="button" onClick={() => setIsSubmitModalOpen(false)} className="px-6 py-2 rounded-xl text-gray-400 hover:text-white hover:bg-white/5 transition-colors">Cancel</button>
                  <button type="submit" className="px-6 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-medium transition-colors">Submit Report</button>
                </div>
              </form>
            </motion.div>
          </div>
