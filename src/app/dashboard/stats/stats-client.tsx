...
export default function StatsClient({ messages, channels, stats }: { messages: MessageStats[]; channels: ChannelStats[]; stats: { totalMessages: number; totalChannels: number; totalMessageRecords: number } }) {
  const [searchTerm, setSearchTerm] = useState("");
  const [tab, setTab] = useState<'messages' | 'channels'>('messages');
  const filteredMessages = messages.filter(m => m.userId.toLowerCase().includes(searchTerm.toLowerCase()) || m.channelId.toLowerCase().includes(searchTerm.toLowerCase()));
  const filteredChannels = channels.filter(c => c.channelId.toLowerCase().includes(searchTerm.toLowerCase()));
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Total Messages", value: stats.totalMessages.toString(), trend: "All time" },
          { label: "Channel Records", value: stats.totalChannels.toString(), trend: "Tracked" },
          { label: "Message Records", value: stats.totalMessageRecords.toString(), trend: "Rows" },
        ].map((stat, i) => (
      ...
      <div className="flex gap-4 mb-2">
        <button onClick={() => setTab('messages')} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${tab === 'messages' ? 'bg-purple-600 text-white' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}>Messages</button>
        <button onClick={() => setTab('channels')} className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${tab === 'channels' ? 'bg-purple-600 text-white' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}>Channels</button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead><tr className="bg-white/5 text-gray-400 text-xs uppercase tracking-wider">
              {tab === 'messages' ? (
                <><th className="p-4 font-medium">User ID</th><th className="p-4 font-medium">Channel ID</th><th className="p-4 font-medium">Count</th><th className="p-4 font-medium">Date</th><th className="p-4 font-medium"></th></>
              ) : (
                <><th className="p-4 font-medium">Channel ID</th><th className="p-4 font-medium">Guild ID</th><th className="p-4 font-medium">Count</th><th className="p-4 font-medium">Date</th><th className="p-4 font-medium"></th></>
              )}
            </tr></thead>
            <tbody className="divide-y divide-white/5">
              {tab === 'messages' ? (
                filteredMessages.length === 0 ? (
                  <tr><td colSpan={5} className="p-8 text-center text-gray-500">No message stats found.</td></tr>
                  ...
                  <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-white/[0.02] transition-colors group cursor-pointer">
                    ...
                    <td className="p-4 text-sm text-gray-300">{item.count}</td>
                    <td className="p-4 text-sm text-gray-400">{new Date(item.date).toLocaleDateString()}</td>
                ...
                filteredChannels.length === 0 ? (
                  <tr><td colSpan={5} className="p-8 text-center text-gray-500">No channel stats found.</td></tr>
                ) : filteredChannels.map((item) => (
                  <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-white/[0.02] transition-colors group cursor-pointer">
                    <td className="p-4 text-sm font-medium text-white">{item.channelId}</td>
                    <td className="p-4 text-sm text-gray-300">{item.guildId}</td>
                    <td className="p-4 text-sm text-gray-300">{item.count}</td>
                    <td className="p-4 text-sm text-gray-400">{new Date(item.date).toLocaleDateString()}</td>
                    <td className="p-4 text-right"><button className="text-gray-500 hover:text-white transition-colors opacity-0 group-hover:opacity-100"><MoreVertical size={18} /></button></td>
                  </motion.tr>
