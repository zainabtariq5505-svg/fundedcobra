...
import { LineChart, Search, Filter, Settings2, MoreVertical, Plus } from "lucide-react";
...
export default function PerformanceClient({ data, stats }: { data: StaffPerformanceSnapshot[]; stats: { total: number; totalClosed: number; avgResponse: number } }) {
  const [searchTerm, setSearchTerm] = useState("");
  const filtered = data.filter(p => p.userId.toLowerCase().includes(searchTerm.toLowerCase()));
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">Analyze ticket metrics and conversions.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Total Snapshots", value: stats.total.toString(), trend: "Recorded" },
          { label: "Tickets Closed", value: stats.totalClosed.toString(), trend: "All time" },
          { label: "Avg Response", value: `${stats.avgResponse}s`, trend: "Per ticket" },
        ].map((stat, i) => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead><tr className="bg-white/5 text-gray-400 text-xs uppercase tracking-wider">
              ...
              <th className="p-4 font-medium">Tickets Claimed</th>
              <th className="p-4 font-medium">Tickets Closed</th>
              <th className="p-4 font-medium">Avg Response</th>
              <th className="p-4 font-medium">Leads Converted</th>
              <th className="p-4 font-medium"></th>
            </tr></thead>
            <tbody className="divide-y divide-white/5">
              {filtered.length === 0 ? (
                <tr><td colSpan={7} className="p-8 text-center text-gray-500">No performance records found.</td></tr>
              ) : filtered.map((item) => (
                <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-white/[0.02] transition-colors group cursor-pointer">
                  ...
                  <td className="p-4 text-sm text-gray-300">{item.ticketsClaimed}</td>
                  <td className="p-4 text-sm text-green-400">{item.ticketsClosed}</td>
                  <td className="p-4 text-sm text-gray-300">{item.avgResponseTime}s</td>
                  <td className="p-4 text-sm text-purple-400">{item.leadsConverted}</td>
