...
import { Zap, Search, Filter, Settings2, MoreVertical, Plus } from "lucide-react";
...
export default function BoostersClient({ data, stats }: { data: BoosterLog[]; stats: { total: number; uniqueBoosters: number; enabledCount: number } }) {
  const [searchTerm, setSearchTerm] = useState("");
  const filtered = data.filter(b => b.userId.toLowerCase().includes(searchTerm.toLowerCase()));
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">Manage server booster perks and announcements.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Total Boosts", value: stats.total.toString(), trend: "All time" },
          { label: "Unique Boosters", value: stats.uniqueBoosters.toString(), trend: "Users" },
          { label: "Enabled Guilds", value: stats.enabledCount.toString(), trend: "Active" },
        ].map((stat, i) => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead><tr className="bg-white/5 text-gray-400 text-xs uppercase tracking-wider">
              ...
              <th className="p-4 font-medium">Boosted At</th>
              <th className="p-4 font-medium"></th>
            </tr></thead>
            <tbody className="divide-y divide-white/5">
              {filtered.length === 0 ? (
                <tr><td colSpan={4} className="p-8 text-center text-gray-500">No booster records found.</td></tr>
              ) : filtered.map((item) => (
