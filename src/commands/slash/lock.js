...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const mc = await createCase({ guildId: interaction.guild.id, actionType: 'lock', target: { id: interaction.channel.id, tag: `#${interaction.channel.name}`, username: `#${interaction.channel.name}` }, moderator: interaction.user, reason });
    await sendModLog(client, interaction.guild.id, buildModEmbed({ caseId: mc.caseId, actionType: 'lock', target: { id: interaction.channel.id, tag: `#${interaction.channel.name}` }, moderator: interaction.user, reason, extra: [{ name: 'Channel', value: `<#${interaction.channel.id}>`, inline: true }] }));
    return interaction.reply({ embeds: [embeds.success(`🔒 <#${interaction.channel.id}> has been **locked**.`)] });
  },
};
